...
//电阻屏芯片连接引脚	   
#define Touch_Port_A   	  	GPIOA	
#define Touch_Port_B   	  	GPIOB	
#define PEN  (Touch_Port_A->IDR &(1<<8))   //PC2  INT
#define DOUT (Touch_Port_B->IDR &(1<<0))   //PC11  MISO
#define TDIN(x) Touch_Port_B->ODR = x ? Touch_Port_B->ODR |(1<<1):Touch_Port_B->ODR &(~(1<<1)) //PC12  MOSI
#define TCLK(x) Touch_Port_A->ODR = x ? Touch_Port_A->ODR |(1<<3):Touch_Port_A->ODR &(~(1<<3)) //PC10  SCLK
#define TCS(x) Touch_Port_A->ODR = x ? Touch_Port_A->ODR |(1<<11):Touch_Port_A->ODR &(~(1<<11))//PC4 CS 
